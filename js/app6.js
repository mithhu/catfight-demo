      ...
      for (let j = 0; j < store.length; j++) {
        if (store[i][j]) {
          ...
          document.getElementById(`${i}${j}`).addEventListener("click", () => {
            moveHandler(`${i}${j}`, id, selectedAvatar, store);
          });
        }
      }
    }
...
const moveHandler = (newId, oldId, selectedAvatar) => {
  ...
  if (
    ...
    console.log(document.getElementById(newId).disabled);
    for (let i = 0; i < store.length; i++) {
      for (let j = 0; j < store.length; j++) {
        store[i][j] = false;
...
const selector = () => {
  ...
  //   document.getElementById(indexA).disabled = true;
  //   document.getElementById(indexB).disabled = true;
